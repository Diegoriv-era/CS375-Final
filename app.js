...
async function getYouTubeSongURL(songName) {
  const apiKey = 'AIzaSyD7DT5B1X7-8YxbMsYQL9HdC2EYuhm7UsA';
  const searchUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(songName)}&key=${apiKey}&type=video&maxResults=1`;
  ...
  try {
    ...
    console.error('Failed to retrieve YouTube song URL:', error);
  }
